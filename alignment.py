...
twod = []
first = blosum.readline().strip().split()
first.insert(0, ' ')
twod.append(first)
for line in blosum:
    l = line.strip().split()
...
for i in range(1, 21):
    ...
    for j in range(1, 21):
        c = twod[0][j]
        blosum_dictionary[(r, c)] = twod[i][j]
...
for i in range(1, len(top_string)):
    for j in range(1, len(bottom_string)):
        ...
        match = score[i - 1][j - 1] + int(blosum_dictionary[temp])
...
best_path = score[len(top_string) - 1][len(bottom_string) - 1]
top_align = top_align[::-1]
bottom_align = bottom_align[::-1]

writer.write(str(best_path) + "\n" + top_align + "\n" + bottom_align)
